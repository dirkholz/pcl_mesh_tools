...
segmentAndClassify (typename pcl::rec_3d_framework::GlobalNNPipeline<DistT, PointT, FeatureT> & global)
{
...

  {
    ...
    if (pcd_files.size() == 0)
      return;
    pcl::io::loadPCDFile(pcd_files[0], *xyz_points);
    ...
    vis.addPointCloud<pcl::PointXYZ> (xyz_points, "frame");
...
main (int argc, char ** argv)
{
  ...
  pcd_file_indices = pcl::console::parse_file_extension_argument(argc, argv, ".pcd");
  for (size_t i = 0; i < pcd_file_indices.size(); ++i)
    pcd_files.push_back(std::string(argv[pcd_file_indices[i]]));
